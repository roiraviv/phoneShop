...
/** תאריך כניסה למלאי + ספק – למכשירים בלבד */
export function normalizePhoneInventoryFields(productData, { isNew = false } = {}) {
  if (productData.productType !== 'phone') return productData;

  const out = { ...productData };
  if (out.supplier != null) {
    out.supplier = String(out.supplier).trim();
  }

  if (isNew) {
    if (!out.stockEnteredAt) {
      out.stockEnteredAt = new Date().toISOString();
    } else if (/^\d{4}-\d{2}-\d{2}$/.test(out.stockEnteredAt)) {
      out.stockEnteredAt = new Date(`${out.stockEnteredAt}T12:00:00`).toISOString();
    }
  }
